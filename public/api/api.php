  ...
  if (isset($_GET['region']) && $_GET['sensor'] && isset($_GET['type'])) {
    switch ($_GET['region']) {
      case 'rooms':
        ...
        if (isset($_GET['number'])) { // Ex: api?region=rooms&number=1&sensor=temperature&type=log
          echo file_get_contents("data/rooms/{$_GET['number']}/{$_GET['sensor']}/{$_GET['type']}.txt") . PHP_EOL;
        } else {
